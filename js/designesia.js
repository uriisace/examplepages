    ...
    function de_share(){
     ...
     $(function(){
        ...
        $(".sub-menu > a").click(function (e) {
            const $submenu = $(this).parent(".sub-menu");
            const hasDropdown = $submenu.children("ul").length > 0;

            if (hasDropdown) {
                e.preventDefault(); // only block links that toggle a submenu

                const currentSubMenu = $submenu.children("ul");
                const icon = $(this).find(".right");

                // Hide all other submenus
                $(".sub-menu ul").not(currentSubMenu).slideUp(300);
                $(".sub-menu a .right").not(icon).removeClass("fa-chevron-up").addClass("fa-chevron-down");
